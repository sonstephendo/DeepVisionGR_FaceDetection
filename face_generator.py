...
class BatchGenerator:
    ...
    def parse_csv(self, labels_path=None, input_format=None, ret=False):
        ...
        with open(self.labels_path) as csvfile:
            ...
            for i in csvread:
                ...
                if k == 0:  # Skip the header row
                    ...
                    if (
                        self.include_classes == "all"
                        or int(i[self.input_format.index("class_id")].strip())
                        ...
                        # Select the image name column in the input format and append its content to `obj`
                        for item in self.box_output_format:
                            # For each item in the output format...
        ...
        for idx, i in enumerate(data):
            if current_file == "":
                # If this is the first image file
                current_file = i[0]
    ...
    def parse_xml(
        ...
        if classes is None:
            classes = []
        if not annotations_path is None:
            self.annotations_path = annotations_path
        if not image_set_path is None:
            self.image_set_path = image_set_path
        if not image_set is None:
        ...
        for key in data:
            ...
            sys.stdout.flush()
            # print(
            #     "reading training image {0} of {1}".format(train_cnt, n_train_samples)
            # )
            ...
            folder = None
            filename = os.path.sep.join((img_path, img_name))
    ...
    def generate(
        ...
        self.filenames, self.labels = shuffle(
        ...
        while True:
            ...
            # The filenames of the files in the current batch
            ...
            # In case we need to remove any images from the batch because of failed random cropping, store their indices in this list
            ...
            for i in range(len(batch_X)):
                ...
                if brightness:
                    ...
                    if p >= (1 - brightness[2]):
                        batch_X[i] = _brightness(
                            batch_X[i], min=brightness[0], max=brightness[1]
                        )
                ...
                if translate:
                    ...
                    if p >= (1 - translate[2]):
                        ...
                        batch_X[i], xshift, yshift = _translate(
                            batch_X[i], translate[0], translate[1]
                        )
                        # Adjust the labels
                        ...
                        if limit_boxes:
                            ...
                            before_area = (
                                before_limiting[:, xmax] - before_limiting[:, xmin]
                            ...
                            if include_thresh == 0:
                                ...
                                # If `include_thresh == 0`, we want to make sure that boxes with area 0 get thrown out, hence the ">" sign instead of the ">=" sign
                            else:
                                batch_y[i] = batch_y[i][
                                    after_area >= include_thresh * before_area
                                ]
                                # Especially for the case `include_thresh == 1` we want the ">=" sign, otherwise no boxes would be left at all

                if scale:
                    p = np.random.uniform(0, 1)
                    if p >= (1 - scale[2]):
                        # Rescale the image and return the transformation matrix M so we can use it to adjust the box coordinates
                        batch_X[i], M, scale_factor = _scale(
                            batch_X[i], scale[0], scale[1]
                        )
                        ...
                        if limit_boxes and (scale_factor > 1):
                            # We don't need to do any limiting in case we shrunk the image
                            before_limiting = deepcopy(batch_y[i])
                            x_coords = batch_y[i][:, [xmin, xmax]]
                            x_coords[x_coords >= img_width] = img_width - 1
                            x_coords[x_coords < 0] = 0
                ...
                if random_crop:
                    ...
                    if p < 0.3:
                        ...
                        trial_counter = 0
                        while (not min_1_object_fulfilled) and (
                            trial_counter < random_crop[3]
                        ):
                            ...
                            if y_range >= 0 and x_range >= 0:
                                # If the patch to be cropped out is smaller than the original image in both dimenstions, we just perform a regular crop
                                # Crop the image
                                patch_X = np.copy(
                                    batch_X[i][
                                        crop_ymin : crop_ymin + random_crop[0],
                                ...
                                if limit_boxes:
                                    ...
                                    y_coords[y_coords < 0] = 0
                                    y_coords[y_coords >= random_crop[0]] = (
                                        random_crop[0] - 1
                                    )
                                    patch_y[:, [ymin, ymax]] = y_coords
                                    x_coords = patch_y[:, [xmin, xmax]]
                                    x_coords[x_coords < 0] = 0
                                    x_coords[x_coords >= random_crop[1]] = (
                                        random_crop[1] - 1
                                    )
                                ...
                                # Crop the image
                                patch_X = np.copy(
                                    batch_X[i][crop_ymin : crop_ymin + random_crop[0]]
                                )
                                # ...crop the vertical dimension just as before,...
                                canvas = np.zeros(
                                    (random_crop[0], random_crop[1], patch_X.shape[2]),
                                    dtype=np.uint8,
                                )
                                ...
                                # If the crop is larger than the original image in both dimensions,...
                                ...
                                canvas[
                                    ...
                                    crop_xmin : crop_xmin + img_width,
                                ] = patch_X
                                # ...and place the patch onto the canvas at the random `(crop_ymin, crop_xmin)` position computed above.
                                patch_X = canvas
                                # Translate the box coordinates into the new coordinate system: In this case, the origin is shifted by `(-crop_ymin, -crop_xmin)`
                                patch_y = np.copy(batch_y[i])
                                patch_y[:, [ymin, ymax]] += crop_ymin
                            ...
                            if random_crop[2] == 0:
                                # If `min_1_object == 0`, break out of the while loop after the first loop because we are fine with whatever crop we got
                                batch_X[i] = patch_X
                                # The cropped patch becomes our new batch item
                                batch_y[i] = patch_y
                                # The adjusted boxes become our new labels for this batch item
                                ...
                                # If we have at least one object left, this crop is valid and we can stop
                            ...
                            ):  # If we've reached the trial limit and still not found a valid crop, remove this image from the batch
                ...
                if resize:
                    ...
                    batch_y[i][:, [xmin, xmax]] = (
                        batch_y[i][:, [xmin, xmax]] * (float(resize[0]) / img_width)
                    ).astype(np.int)
                    batch_y[i][:, [ymin, ymax]] = (
                        batch_y[i][:, [ymin, ymax]] * (float(resize[1]) / img_height)
                    ).astype(np.int)
                    ...
                    img_width, img_height = (
                        resize
                    )  # Updating these at this point is unnecessary, but it's one fewer source of error if this method gets expanded in the future

                if gray:
                    batch_X[i] = np.expand_dims(
                        cv2.cvtColor(batch_X[i], cv2.COLOR_RGB2GRAY), 3
                    )
            ...
            for j in sorted(batch_items_to_remove, reverse=True):
                batch_X.pop(j)
                batch_y.pop(j)
                # This isn't efficient, but this should hopefully not need to be done often anyway

            if train:
                # During training we need the encoded labels instead of the format that `batch_y` has
                if ssd_box_encoder is None:
                    raise ValueError(
                        "`ssd_box_encoder` cannot be `None` in training mode."
                    )
    ...
    def process_offline(
        ...
        for k, filename in enumerate(self.filenames[start:stop]):
            i = k + start
            with Image.open(
                "{}".format(os.path.join(self.images_path, filename))
            ) as img:
                image = np.array(img)
            ...
            if flip:
                ...
                if p >= (1 - flip):
                    image = _flip(image)
                    targets[:, [0, 1]] = (
                        img_width - targets[:, [1, 0]]
                    )  # xmin and xmax are swapped when mirrored

            if translate:
                p = np.random.uniform(0, 1)
                if p >= (1 - translate[2]):
                    image, xshift, yshift = _translate(
                        image, translate[0], translate[1]
                    )
                    targets[:, [0, 1]] += xshift
                    targets[:, [2, 3]] += yshift
                    if limit_boxes:
                        ...
                        # `include_thresh` of the box area before limiting.
                        before_area = (
                            before_limiting[:, 1] - before_limiting[:, 0]
                        ) * (before_limiting[:, 3] - before_limiting[:, 2])
                        after_area = (targets[:, 1] - targets[:, 0]) * (
                            targets[:, 3] - targets[:, 2]
                        )
            ...
            if scale:
                ...
                if p >= (1 - scale[2]):
                    ...
                    # Transform two opposite corner points of the rectangular boxes using the transformation matrix `M`
                    toplefts = np.array(
                        [targets[:, 0], targets[:, 2], np.ones(targets.shape[0])]
                    )
                    bottomrights = np.array(
                        [targets[:, 1], targets[:, 3], np.ones(targets.shape[0])]
                    )
                    new_toplefts = (np.dot(M, toplefts)).T
                    new_bottomrights = (np.dot(M, bottomrights)).T
            ...
            if crop:
                image = image[
                    crop[0] : img_height - crop[1], crop[2] : img_width - crop[3]
                ]
                if limit_boxes:
                    # Adjust boxes affected by cropping and remove those that will no longer be in the image
                    before_limiting = np.copy(targets)
                    if crop[0] > 0:
                        y_coords = targets[:, [2, 3]]
                    ...
                    if crop[1] > 0:
                        y_coords = targets[:, [2, 3]]
                        y_coords[y_coords >= (img_height - crop[1])] = (
                            img_height - crop[1] - 1
                        )
                        targets[:, [2, 3]] = y_coords
                    if crop[2] > 0:
                    ...
                    if crop[3] > 0:
                        x_coords = targets[:, [0, 1]]
                        x_coords[x_coords >= (img_width - crop[3])] = (
                            img_width - crop[3] - 1
                        )
                        targets[:, [0, 1]] = x_coords
                    # Some objects might have gotten pushed so far outside the image boundaries in the transformation
            ...
            if resize:
                ...
                targets[:, [0, 1]] = (
                    targets[:, [0, 1]] * (resize[0] / img_width)
                ).astype(np.int)
                targets[:, [2, 3]] = (
                    targets[:, [2, 3]] * (resize[1] / img_height)
                ).astype(np.int)
