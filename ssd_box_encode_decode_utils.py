...
def iou(boxes1, boxes2, coords="centroids"):
    ...
    if coords == "centroids":
        ...
        boxes1 = convert_coordinates(
            boxes1, start_index=0, conversion="centroids2minmax"
        )
        boxes2 = convert_coordinates(
            boxes2, start_index=0, conversion="centroids2minmax"
        )
    elif coords != "minmax":
        raise ValueError(
    ...
    intersection = np.maximum(
        0,
        np.minimum(boxes1[:, 1], boxes2[:, 1]) - np.maximum(boxes1[:, 0], boxes2[:, 0]),
    ) * np.maximum(
        0,
        np.minimum(boxes1[:, 3], boxes2[:, 3]) - np.maximum(boxes1[:, 2], boxes2[:, 2]),
    )
    union = (
        (boxes1[:, 1] - boxes1[:, 0]) * (boxes1[:, 3] - boxes1[:, 2])
...
def decode_y(
    ...
    if input_coords == "centroids":
        ...
        y_pred_decoded_raw = convert_coordinates(
            y_pred_decoded_raw, start_index=-4, conversion="centroids2minmax"
        )
    ...
    for batch_item in y_pred_decoded_raw:
        ...
        for class_id in range(1, n_classes):
            ...
            if threshold_met.shape[0] > 0:
                ...
                maxima = _greedy_nms(
                    threshold_met, iou_threshold=iou_threshold, coords="minmax"
                )
                # ...perform NMS on them.
...
def decode_y2(
    ...
    if input_coords == "centroids":
        ...
        y_pred_converted = convert_coordinates(
            y_pred_converted, start_index=-4, conversion="centroids2minmax"
        )
    elif input_coords == "minmax":
...
class SSDBoxEncoder:
    ...
    def generate_anchor_boxes(
        ...
        cx = np.linspace(
            cell_width / 2, self.img_width - cell_width / 2, feature_map_size[1]
        )
        cy = np.linspace(
            cell_height / 2, self.img_height - cell_height / 2, feature_map_size[0]
        )
    ...
    def generate_encode_template(self, batch_size, diagnostics=False):
        ...
        if diagnostics:
            wh_list = []
            # List to hold the box widths and heights
            cell_sizes = []
            # List to hold horizontal and vertical distances between any two boxes
            if self.aspect_ratios_per_layer:
                # If individual aspect ratios are given per layer, we need to pass them to `generate_anchor_boxes()` accordingly
                ...
                # Use the same global aspect ratio list for all layers
                for i in range(len(self.predictor_sizes)):
                    boxes, wh, cells = self.generate_anchor_boxes(
                        batch_size=batch_size,
                        feature_map_size=self.predictor_sizes[i],
    ...
    def encode_y(self, ground_truth_labels):
        ...
        for i in range(y_encode_template.shape[0]):
            ...
            for true_box in ground_truth_labels[i]:
                ...
                similarities = iou(
                    y_encode_template[i, :, -12:-8], true_box[1:], coords=self.coords
                )
                ...
                available_and_thresh_met[
                    available_and_thresh_met < self.pos_iou_threshold
                ] = 0
                ...
                if len(assign_indices) > 0:
                    ...
                    # Make the assigned anchor box unavailable for the next ground truth box
        ...
        if self.coords == "centroids":
            ...
            y_encoded[:, :, [-12, -11]] /= (
                y_encode_template[:, :, [-10, -9]] * y_encode_template[:, :, [-4, -3]]
            )
            ...
            y_encoded[:, :, [-10, -9]] = (
                np.log(y_encoded[:, :, [-10, -9]]) / y_encode_template[:, :, [-2, -1]]
            )
