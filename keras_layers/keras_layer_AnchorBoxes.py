...
class AnchorBoxes(Layer):
    ...
    def call(self, x, mask=None):
        ...
        if K.image_dim_ordering() == "tf":
            ...
            batch_size, feature_map_channels, feature_map_height, feature_map_width = (
                x._keras_shape
            )
        ...
        cx = np.linspace(
            cell_width / 2, self.img_width - cell_width / 2, feature_map_width
        )
        cy = np.linspace(
            cell_height / 2, self.img_height - cell_height / 2, feature_map_height
        )
