...
def parse_args():
    parser = argparse.ArgumentParser(description='Get WIDER filenames from WIDER annotations')
    parser.add_argument('-ap', dest='annotations_path', required=True,
                        help='the annotations file path. ie: "-ap ./wider_face_split/wider_face_train_bbx_gt.txt".')
    parser.add_argument('-o', dest='output', required=True,
                        help='the target output files will be saved. ie: "-o ./WIDER_train/wider_face_train_filelist.txt"')
    parser.add_argument('-l', dest='limit',
                        help='The exactly size of image list. ie: 100')
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
